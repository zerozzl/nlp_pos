...
class WordPOS(nn.Module):
    def __init__(self, num_tags, word_vocab_size, word_embed_size, input_dropout_rate, char_cnn_kernel_size, hidden_dim,
                 hidden_dropout_rate, use_char_embed, char_vocab_size, char_embed_size, use_crf, embed_freeze):
        ...
        if use_char_embed:
            ...
            self.char_cnn = nn.Conv1d(in_channels=char_embed_size, out_channels=char_embed_size,
    ...
    def forward(self, tokens, masks, chars=None, decode=True, tags=None):
        out = self.word_embedding(tokens)

        if self.use_char_embed:
            batch_size, max_sent_len, max_word_len = chars.size()
            chars = chars.reshape(batch_size, -1)
            char_emb = self.char_embedding(chars)
            char_emb = char_emb.permute(0, 2, 1)
            char_emb = self.char_cnn(char_emb)
            char_emb = char_emb.permute(0, 2, 1)
            char_emb = char_emb.reshape(batch_size, max_sent_len, max_word_len, -1)
            char_emb = F.max_pool2d(char_emb, kernel_size=(char_emb.size(2), 1))
            char_emb = char_emb.squeeze(2)
            out = torch.cat((out, char_emb), dim=2)
