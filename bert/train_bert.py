...
from bert.model import CharPOS
from utils.utils import setup_seed
from utils.dataloader import load_tag_dict, convert_ids_to_tags, BaseDataset
from utils import modelloader, evaluator


def get_dataset(args, tokenizer, tag_to_id):
    train_dataset = BaseDataset('%s/%s/train_char.txt' % (args.data_path, args.task), 'char', args.max_inp_size,
                                do_pad=True, do_to_id=True, tag_to_id=tag_to_id, char_tokenizer=tokenizer,
                                for_bert=True, debug=args.debug)
    dev_dataset = BaseDataset('%s/%s/dev_char.txt' % (args.data_path, args.task), 'char', args.max_inp_size,
                              do_pad=True, do_to_id=True, tag_to_id=tag_to_id, char_tokenizer=tokenizer,
                              for_bert=True, debug=args.debug)
    test_dataset = BaseDataset('%s/%s/test_char.txt' % (args.data_path, args.task), 'char', args.max_inp_size,
                               do_pad=True, do_to_id=True, tag_to_id=tag_to_id, char_tokenizer=tokenizer,
                               for_bert=True, debug=args.debug)
...
def data_collate_fn(data):
    ...
    return sents, pos_tag, masks, segments, sents_len
...
def evaluate(args, dataloader, id_to_tag, model):
    ...
    _, pre, rec, f1 = evaluator.evaluate(gold_answers, pred_answers)
    return pre, rec, f1
...
def main(args):
    ...
    tag_to_id, id_to_tag = load_tag_dict('%s/%s/tags_char.txt' % (args.data_path, args.task))
    ...
    if args.pretrained_model_path is not None:
        ...
        model = CharPOS('%s/config.json' % args.pretrained_bert_path,
    ...
    while epoch < args.epoch_size:
        ...
        if dev_f1 > best_f1:
            ...
            test_pre, test_rec, test_f1 = evaluate(
                args, test_dataloader, id_to_tag, model)
