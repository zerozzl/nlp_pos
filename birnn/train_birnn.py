...
from birnn.model import CharPOS, WordPOS
...
def get_dataset(args, tag_to_id, char_tokenizer, bigram_tokenizer, word_tokenizer):
    train_dataset = BaseDataset('%s/%s/train_%s.txt' % (args.data_path, args.task, args.use_char_or_word),
                                args.use_char_or_word, args.max_sent_len,
                                do_pad=False, do_to_id=True, tag_to_id=tag_to_id, char_tokenizer=char_tokenizer,
                                add_bigram_feature=args.use_bigram_embed, bigram_tokenizer=bigram_tokenizer,
                                word_tokenizer=word_tokenizer, do_sort=True, debug=args.debug)
...
def data_collate_fn(data):
    ...
    if data_shape[1] == 6:  # char
        ...
        bigrams = data[:, 1].tolist()
        bigrams = [torch.LongTensor(np.array(s)) for s in bigrams]
        bigrams = rnn.pad_sequence(bigrams, batch_first=True)

        seg_tags = data[:, 2].tolist()
        seg_tags = [torch.LongTensor(np.array(s)) for s in seg_tags]
        seg_tags = rnn.pad_sequence(seg_tags, batch_first=True)

        pos_tags = data[:, 3].tolist()
        pos_tags = [torch.LongTensor(np.array(s)) for s in pos_tags]
        pos_tags = rnn.pad_sequence(pos_tags, batch_first=True)
        ...
        return sents, bigrams, seg_tags, pos_tags, masks, sents_len
    if data_shape[1] == 5:
        sents = data[:, 0].tolist()
        sents = [torch.LongTensor(np.array(s)) for s in sents]
        sents = rnn.pad_sequence(sents, batch_first=True)

        pos_tags = data[:, 2].tolist()
        pos_tags = [torch.LongTensor(np.array(s)) for s in pos_tags]
        pos_tags = rnn.pad_sequence(pos_tags, batch_first=True)
        ...
        return sents, pos_tags, masks, sents_len
...
def train(args, dataset, dataloader, model, optimizer, lr_scheduler):
    ...
    for batch, data in enumerate(dataloader):
        ...
        if args.use_char_or_word == 'word':
            sents, pos_tag, masks, _ = data

            sents = sents.cpu() if args.use_cpu else sents.cuda()
            pos_tag = pos_tag.cpu() if args.use_cpu else pos_tag.cuda()
            masks = masks.cpu() if args.use_cpu else masks.cuda()

            loss = model(sents, masks, decode=False, tags=pos_tag)
...
def evaluate(args, dataloader, id_to_tag, model):
    ...
    with torch.no_grad():
        for batch, data in enumerate(dataloader):
            ...
            if args.use_char_or_word == 'word':
                sents, pos_tag, masks, sents_len = data
    ...
    if args.pretrained_model_path is not None:
        ...
        if args.use_char_or_word == 'char':
            model = CharPOS(len(tag_to_id), len(char_to_id), args.char_embed_size, args.input_dropout_rate,
                            args.hidden_layers, args.hidden_dim, args.hidden_dropout_rate,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hidden_dropout_rate', dest='hidden_dropout_rate', type=float,
                        default=0.5)
    parser.add_argument('--hidden_layers', dest='hidden_layers', type=int,
                        default=1)
    ...
    parser.add_argument('--learning_rate', dest='learning_rate', type=float,
                        help='0.01 without crf, 0.0001 with crf',
                        default=0.01)
